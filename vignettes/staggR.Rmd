...
`ts_plot`. It has three {>>(main? or more?)<<} arguments, ... . 
...
function `ts_plot()` omits any counties where any of the elements in the formula above (`hospitalized ~ county + yr`) or
the `intervention_var` are `NA`.{>>Kyle: should this be `intervention_yr`?<<}
...
Another important argument of the `sdid()` function `intervention_var` for the name of the column that contains the time
period during which each cohort implemented the intervention. In our case, this is the column called `intervention_yr`.

Furthermore, the argument `df` is used to specific the data. 

Thus, call for estimating the staggered difference-in-differences regression with age, sex, and comorbitiy as covariates
is as follows:
...
We can use `cohort_time_refs` the argument to specify a different time period to function as the referent for
cohort-time interactions, as shown next:
...
The `ave_coeff()` function calculates averages of any specified set of regression coefficients along with corresponding
standard errors. It has two primary argumnets:

- `sdid` supplies the object from the `sdid` function

- `period` specifies the period for which we want to calculate the average. 

{>>I think here we need to describe hoe the function `select_period` works; maybe start with that one?<<}
...
The result shows that there is a positive association between the intervention and increase in the probability of
hospitalization. 
...
We can also use the `ave_coeff()` function to examine whether there exists heterogeneity in difference-in-differences
estimates between various cohorts, using the argument `cohort`. For instance, we could calculate the average of
difference-in-differences estimates for the post-intervention period for cohorts 5 and 6 as follows:
...
Comparing these averages shows that the association between the intervention and changes in the probability of
hospitalization for the last two cohorts is almost twice as large than the for the first two cohorts.

We can further aggregate coefficients belonging to a specific time suing the function `select_terms` with the argument
`selection`. For example, we could calculate the added risk of hospitalization associated with the intervention across
all cohorts for the year 2018.

```{r}
(terms_2018 <- staggR::select_terms(mdl = sdid_hosp,
...
Alternative, we could calculate the added risk of hospitalization associated with the intervention in the year 2018, but
only for the first two cohorts (5 and 6).

{>>Kyle: We should describe the function `select_terms` here or above, when we introduce the function `select_period`.<<}

```{r}
(terms_2018_cohorts56 <- staggR::select_terms(mdl = sdid_hosp,
...
```{r}
(terms_custom <- staggR::select_terms(mdl = sdid_hosp,
...
dependent variable, and covariates are their aggregated counterparts (i.e., `mean_age`, `pct_fem`, and `pct_cmb`). The
main difference is that we need to specifcy the `weights` argument for the number of observations contained within each
...
Aggregating means that we decoupled values of these covariates from outcomes within county-years, so coefficient are
slightly different but substantially similar compared to the regression using individual-level data. 
...
```{r}
sdid_hosp_agg2 <- staggR::sdid(pct_hospitalized ~ cohort + yr + mean_age + pct_fem + pct_cmb,
                               df = hosp_agg,
                               weights = "n_enr",
                               intervention_var  = "intervention_yr",
...
staggR::ave_coeff(sdid = sdid_hosp_agg2, 
                  coefs = staggR::select_period(sdid = sdid_hosp_agg2,
                                                period = "post"))
```
