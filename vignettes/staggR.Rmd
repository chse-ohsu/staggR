...
If, for some reason, we wanted a different time period to function as the referent for cohort-time interactions, we could specify that using the `cohort_time_refs` argument.
...
```{r}
# Identify the coefficients that compose the effect of the intervention during the post-intervention period
...
Aside from aggregating the entire pre- or post-intervention periods for specific cohorts, we might want to aggregate a different set of coefficients. For example, we could calculate the risk of hospitalization across all cohorts in the year 2018. 
...
Or maybe we want the risk of hospitalization in the year 2018, but only for the first two cohorts (5 and 6). 
...
summary_sdid_hosp <- 
  broom::tidy(sdid_hosp$mdl)[, c("term", "estimate", "std.error")]

# Summarize the model fitted using aggregated data
summary_sdid_hosp_agg <- 
  broom::tidy(sdid_hosp_agg$mdl)[, c("estimate", "std.error")]

# Combine the two summaries side by side
colnames(summary_sdid_hosp_agg) <- c("agg_estimate", "agg_se")
cbind(summary_sdid_hosp, summary_sdid_hosp_agg)
```

### Calculating custom standard errors

If we want standard errors clustered at the county level, we might specify that we want to use the clustered covariance matrix estimation function from the `sandwich` package. [CAN WE ADD SOMETHING HERE ABOUT WHY WE WOULD DO THIS?]
